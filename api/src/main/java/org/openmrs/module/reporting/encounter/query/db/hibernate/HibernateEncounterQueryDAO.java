...
public class HibernateEncounterQueryDAO implements EncounterQueryDAO {
	 ...
	 *      org.openmrs.module.reporting.common.TimeQualifier) Caveat for this implementation: For @param
	 *      encounterDatetimeOnOrBefore if time is not set (00:00:00), then time is changed to
	 *      23:59:59
	 */
	public List<Encounter> getEncounters(Cohort cohort, List<EncounterType> encounterTypes, List<Form> forms,
		...
		if (cohort != null && cohort.size() == 0) {
			...
			if (encounterDatetimeOnOrBefore != null)
				criteria.add(Expression.le("encounterDatetime",
				    DateUtil.getEndOfDayIfTimeExcluded(encounterDatetimeOnOrBefore)));
