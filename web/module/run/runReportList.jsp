...
<c:if test="${fn:length(reports) != 0}">
	...
	<table cellpadding="2" cellspacing="0">
		<tr>
			<th></th>
			<th style="padding-left: 3em"><spring:message code="reporting.Report.description"/></th>
			<th style="padding-left: 3em"><spring:message code="reporting.Report.list.lastRun"/></th>
		</tr>
		<c:forEach var="report" items="${reports}" varStatus="status">
			<tr valign="baseline" class="<c:choose><c:when test="${status.index % 2 == 0}">evenRow</c:when><c:otherwise>oddRow</c:otherwise></c:choose>">
				<td>
					<a href="runReport.form?reportId=${report.uuid}">${report.name}</a></div>
				</td>
