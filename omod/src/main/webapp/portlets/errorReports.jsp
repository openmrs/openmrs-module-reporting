...
<%--
	available properties:
		...
		showIfNone -> (boolean, default true) whether to display an empty box is there are no error reports
--%>
	...
	<c:if test="${showDecoration}">
		<div class="portlet">
			<div class="portlet-header">
				<spring:message  code="reporting.Report.failed.title" />
			</div>
			<div class="portlet-content">
	</c:if>
	
	<c:if test="${fn:length(model.errorRequests) == 0}">
		<spring:message code="reporting.none"/>
	</c:if>
	
	<table cellspacing="0" cellpadding="2">
		<c:forEach var="r" items="${model.errorRequests}" varStatus="iterstatus">
			<tr valign="top">
				<td <c:if test="${!iterstatus.last}"> style="border-bottom: 1px #c0c0c0 solid; white-space: nowrap;" </c:if>>
					...
					<c:if test="${r.description != null}">
						<span class="small">
							<span class="faded">
								<spring:message code="reporting.Report.description" />
							</span>
							${r.description}
						</span>
					</c:if>
