				...
				<ul>				
					...
					<c:forEach var="parameter" items="${indicatorForm.cohortIndicator.parameters}">				
						<li>
							...
							<div>						
								<wgt:widget id="${parameter.name}" name="parameterValues[${parameter.name}]" defaultValue="<%=WebUtil.encodeForHtmlAttribute(new java.util.Date().toString()) %>" type="${parameter.type.name}"/>
							</div>
						</li>						
					</c:forEach>
					<li>					
